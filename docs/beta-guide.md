...
> Superseded: この参加者ガイドは hosted Seller / 独自
> `subly-yield-exact` β向けの古い配布文書である。現在の標準導線は
> 既存 x402 Seller に対する `@subly_fi/pay` の Buyer-side payment であり、
> セットアップは `packages/pay/README.md` を参照する。
