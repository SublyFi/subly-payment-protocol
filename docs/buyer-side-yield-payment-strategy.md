...
### 1. Legacy: Subly Yield-Attested Seller Mode

過去の `subly-yield-exact` に近い。現在の主導線ではない。
...
- 履歴・検証用の legacy mode。
- 既存 x402 Seller をそのまま使える、という現在の価値提案とは衝突するため、
  最終デモやGTMの主導線では使わない。
...
Seller SDK / `subly-yield-exact` / Subly facilitator / hosted Seller flow は
現在のGTM主導線から外し、legacy reference として扱う。
