...
現在の主導線は standard x402 のみを正とする。旧 `subly-yield-exact`
Seller flow への fallback は、既存x402 Sellerをそのまま使えるという
価値提案を曖昧にするため、最終デモやGTMの主導線には含めない。

```text
paidFetch(url)
  1. request without API key
  2. if 402 has exact + solana USDC -> standard x402 payer flow
  3. otherwise reject no_supported_requirement
```
...
追加すること。`subly-yield-exact` / Subly facilitator / hosted Seller flow は
履歴・検証用の legacy path として扱い、GTM の主導線から外す。最終デモでは、
Nansen のような既存 x402 Seller をそのまま使える Buyer-side payer mode を正とする。
