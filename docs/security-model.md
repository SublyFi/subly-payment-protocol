...
Mandates and approvals are scoped to `(wallet, vault)`. The default enforced policy requires owner approval for deposits and allows normal withdrawals. Owners may require withdrawal approval. A revoked mandate blocks new relayer operations including withdrawals; only the same owner credential can restore it. Client and relayer 0.8.3+ provide owner management links that retain the existing credential and bind to the exact current mandate state. Stale links and replayed actions are refused. Lost-credential recovery has a 72-hour grace period with current-owner cancellation, and cannot override explicit revocation. Expired mandates fall back to the documented default policy; they are not an on-chain freeze. Operators must keep `SUBLY_MANDATE_ENFORCEMENT=on` for intended policy enforcement.
