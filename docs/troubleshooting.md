...
Start with `npx -y @subly_fi/pay@0.8.2 doctor` and record the exact client version with `npx -y @subly_fi/pay@0.8.2 --version`. Continue only after `doctor` reports `"ok": true`; this checks configuration and reachability, not balances, yield, simulation support or vault safety. For first-time setup, use the [client guide](../packages/pay/README.md), [getting-started guide](getting-started.md) or [AI setup prompts](ai-setup-prompts.md).
...
| `pay: command not found` | Use the versioned invocation, for example `npx -y @subly_fi/pay@0.8.2 status <intentId>`. Older output may show a bare command; do not repeat a deposit or withdrawal just to get new instructions. |
...
| Relayer or RPC is unreachable | Replace placeholder endpoints with your trusted operator/RPC URLs, check mainnet configuration and run `npx -y @subly_fi/pay@0.8.2 doctor` again. This guide supplies no guaranteed public relayer. |
...
| Symptom | Action |
| --- | --- |
| `mandate_required_for_deposit` | Run `npx -y @subly_fi/pay@0.8.2 setup-link --initial-deposit <rawAmount>`, have the owner approve, then run `npx -y @subly_fi/pay@0.8.2 setup-status <sessionId>`. Continue only on `status: completed`; deposit the same amount while the initial approval is valid. |
...
| `approvalRequired` / `approval_required` | Open the original `approveUrl`, approve, and retry the same operation with its `apr_...` ID. For example: `npx -y @subly_fi/pay@0.8.2 deposit 1010000 apr_YOUR_APPROVAL_ID`. For API payment, keep the same URL, method, body, headers and cap. Replace the example ID. |
...
| `insufficient_yield` | Run `npx -y @subly_fi/pay@0.8.2 budget` and inspect `spendableYieldRawUsdc`. Principal cannot fund API payments. Wait for sufficient yield plus fees; there is no guaranteed waiting time and retries do not create yield. A small setup deposit is not an immediate paid-call demo. |
...
| Symptom | Action |
| --- | --- |
| `submitted` / confirmation timeout | Run `npx -y @subly_fi/pay@0.8.2 status <intentId>` or MCP `check_subly_vault_operation` with the original `dep_...` or `wdr_...` ID. Requires relayer 0.8.0 or newer; reconciles without rebroadcasting. `confirmed` / `done` is success; `submitted` / `check_again` means check that same ID later. Never repeat a deposit or withdrawal while it may still confirm. |
...
| `vault_flow_pending` during sync | Reconcile the IDs in the error details with `npx -y @subly_fi/pay@0.8.2 status <intentId>` or the MCP status tool, then refresh the budget. Sync will not classify those funds as an external deposit or withdrawal. |
...
| `stale_position_snapshot` | Refresh with `npx -y @subly_fi/pay@0.8.2 budget`. Another operation updated the ledger during the old read, or the RPC is behind the ledger's last observed slot. If it persists, ask the operator to check the RPC and ledger. |
