...
Start with `npx -y @subly_fi/pay@0.8.5 doctor` and record the exact client version with `npx -y @subly_fi/pay@0.8.5 --version`. Continue only after `doctor` reports `"ok": true`; this checks configuration and reachability, not balances, yield, simulation support or vault safety. For first-time setup, use the [client guide](../packages/pay/README.md), [getting-started guide](getting-started.md) or [AI setup prompts](ai-setup-prompts.md).
...
| `pay: command not found` | Use the versioned invocation, for example `npx -y @subly_fi/pay@0.8.5 status <intentId>`. Older output may show a bare command; do not repeat a deposit or withdrawal just to get new instructions. |
...
| Relayer or RPC is unreachable | Replace placeholder endpoints with your trusted operator/RPC URLs, check mainnet configuration and run `npx -y @subly_fi/pay@0.8.5 doctor` again. This guide supplies no guaranteed public relayer. |
...
| Symptom | Action |
| --- | --- |
| `mandate_required_for_deposit` | Run `npx -y @subly_fi/pay@0.8.5 setup-link --initial-deposit <rawAmount>`, have the owner approve, then run `npx -y @subly_fi/pay@0.8.5 setup-status <sessionId>`. Continue only on `status: completed`; deposit the same amount while the initial approval is valid. |
| Owner link is `http://localhost/...` | Open it on the machine running the relayer. On a phone or another computer, `localhost` refers to that device. Do not rewrite a passkey link to an IP address or a different domain; the credential is bound to its original hostname. For remote access, the operator must configure a reachable HTTPS origin and plan owner credential migration. |
...
| `approvalRequired` / `approval_required` | Open the original `approveUrl`, approve, and retry the same operation with its `apr_...` ID. For example: `npx -y @subly_fi/pay@0.8.5 deposit 1010000 apr_YOUR_APPROVAL_ID`. For API payment, keep the same URL, method, body, headers and cap. Replace the example ID. |
...
| Reactivated mandate is already expired | Generate an `owner-link` with `--ttl-days` and approve with the current owner credential; omitted expiry is preserved. Confirm the resulting effective status. Expiry falls back to the default policy, so it does not stop all spending. |
| Cap exceeded despite approval | Approval does not raise per-payment, rolling 24-hour or rolling 30-day caps and does not bypass the payee list. Check previous usage and update the policy through the existing owner when appropriate. Equality is allowed: current usage plus requested amount must be at most the cap. A client cap such as `10000` is separate from the mandate. |
| New setup lost previous restrictions | A new setup uses defaults for omitted fields; it does not inherit an old mandate. The setup CLI/MCP cannot set the payee list, monthly cap or deposit/withdrawal policy. Use owner management to set these fields before further operations; changing policy invalidates old approvals. |
...
| `insufficient_yield` (CLI `fetch`: `realize_failed` with this `detail.error.code`) | Run `npx -y @subly_fi/pay@0.8.5 budget` and inspect `spendableYieldRawUsdc`. Principal cannot fund API payments. Wait for sufficient yield plus fees; there is no guaranteed waiting time and retries do not create yield. A small setup deposit is not an immediate paid-call demo. |
| Paid API returns an application error | A valid x402 settlement may already have charged the request. Preserve the pending state, verify the original outcome with the seller and check the request schema before another paid attempt. An HTTP error is not proof that no funds moved. |
...
| Symptom | Action |
| --- | --- |
| `submitted` / confirmation timeout | Run `npx -y @subly_fi/pay@0.8.5 status <intentId>` or MCP `check_subly_vault_operation` with the original `dep_...` or `wdr_...` ID. Requires relayer 0.8.0 or newer; reconciles without rebroadcasting. `confirmed` / `done` is success; `submitted` / `check_again` means check that same ID later. Never repeat a deposit or withdrawal while it may still confirm. |
...
| `vault_flow_pending` during sync | Reconcile the IDs in the error details with `npx -y @subly_fi/pay@0.8.5 status <intentId>` or the MCP status tool, then refresh the budget. Sync will not classify those funds as an external deposit or withdrawal. |
...
| `stale_position_snapshot` | Refresh with `npx -y @subly_fi/pay@0.8.5 budget`. Another operation updated the ledger during the old read, or the RPC is behind the ledger's last observed slot. If it persists, ask the operator to check the RPC and ledger. |
