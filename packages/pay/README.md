...
```sh
npx -y @subly_fi/pay@0.8.3 --version
npx -y @subly_fi/pay@0.8.3 --help
```

The version should be `0.8.3`. A help screen alone does not check your wallet or endpoints.
...
npx -y @subly_fi/pay@0.8.3 vaults
```
...
```sh
npx -y @subly_fi/pay@0.8.3 setup-status st_YOUR_SESSION_ID
```
...
```sh
npx -y @subly_fi/pay@0.8.3 setup-link --help
```
...
```sh
npx -y @subly_fi/pay@0.8.3 fetch https://seller.example.com/paid-resource
```

The default client cap is **0.01 USDC**. An explicit cap of 0.02 USDC looks like this:

```sh
npx -y @subly_fi/pay@0.8.3 fetch https://seller.example.com/paid-resource 20000
```
...
```sh
npx -y @subly_fi/pay@0.8.3 fetch https://seller.example.com/paid-resource 20000 apr_YOUR_APPROVAL_ID
```
...
```sh
npx -y @subly_fi/pay@0.8.3 withdraw 1000000 apr_YOUR_APPROVAL_ID
```

Later deposits use the same approval pattern:

```sh
npx -y @subly_fi/pay@0.8.3 deposit 1010000 apr_YOUR_APPROVAL_ID
```
...
```sh
npx -y @subly_fi/pay@0.8.3 status wdr_YOUR_WITHDRAWAL_ID
```
...
```sh
npx -y @subly_fi/pay@0.8.3 recovery-status
npx -y @subly_fi/pay@0.8.3 owner-link --per-payment-cap 2000000
npx -y @subly_fi/pay@0.8.3 owner-status st_YOUR_SESSION_ID
```
...
```sh
npx -y @subly_fi/pay@0.8.3 recovery-start
npx -y @subly_fi/pay@0.8.3 recovery-status
```
...
## Run an unpublished source checkout

Check registry availability before using a pinned package:

```sh
npm view @subly_fi/pay@0.8.3 version
```

If this candidate has not been published, run the following from this reviewed
repository checkout. This builds the same client without claiming a registry
release exists:

```sh
npm ci --ignore-scripts
npm ci --prefix packages/pay --ignore-scripts
npm run build --prefix packages/pay
node packages/pay/dist/cli.js --version
node packages/pay/dist/cli.js --help
```

Replace the `npx -y @subly_fi/pay@0.8.3` prefix in this guide with
`node packages/pay/dist/cli.js`. For MCP, launch `node` with the absolute path to
that file followed by `mcp`. Run the matching source relayer for the new owner
commands. Do not mix an old server with new management commands.
...
- For an interrupted manual deposit or withdrawal, use `npx -y @subly_fi/pay@0.8.3 status <intentId>` or MCP `check_subly_vault_operation` with the original ID and configuration, rather than repeating the financial operation.
