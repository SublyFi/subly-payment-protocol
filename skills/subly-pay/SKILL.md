  ...
  openclaw:
    ...
    envVars:
      - name: SUBLY_DEMO_AGENT_KEYPAIR_PATH
        required: true
        description: Path to the agent wallet keypair JSON (generated by demo/setup-beta.sh). The private key never leaves this file.
...
one-shot pay command with the resource URL:

```bash
npm run demo:pay -- "<url>"
```

To set a tighter per-call cap (raw USDC, 6 decimals — e.g. 100 = 0.0001 USDC):

```bash
npm run demo:pay -- "<url>" 100
```
